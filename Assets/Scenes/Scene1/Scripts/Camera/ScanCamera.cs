...
public class ScanCamera : MonoBehaviour
{
    ...
    private void CameraFix()
    {
        ...
        ABVector.y = 0;
        float angle = Vector3.Angle(ABVector, GlassRotate*Vector3.forward);
        Debug.Log(angle);
        if (FixAxis == "X")
        {
            if (Distance < 8)
            {
                ...
                transform.position = Vector3.Lerp(transform.position, new Vector3(Mathf.Clamp(CharacterCamera.transform.position.x, OriginAxisPosition.x + MinP * (Mathf.Clamp((1 / Distance),0,1)), OriginAxisPosition.x + MaxP * (Mathf.Clamp((1 / Distance), 0, 1))), transform.position.y, transform.position.z), 1f);
            }
            else
            {
